...
#define R0 9.83
float Ro = 10.0;
float curve[3] ={2.3, 0.53, -0.43};
float getPpm(uint16_t adc) {
  float res = RL*(1023.0 - adc)/adc;
  res = res/Ro;
  float ppm = pow(10, (((log(res) - curve[1])/curve[2]) + curve[0]));
  return ppm;
}
...
NodeComplexData data = {0, 0.0, 0.0, 0.0, 0};
...
void loop() {
  ...
  if (millis() - displayTimer >= 2000) {
    displayTimer = millis();
    data.count++;
    //data.count = EEPROM.read(eeAdr + 1) +1;
    //EEPROM.write(eeAdr + 1, data.count);
    //data.id = mesh.getNodeID();
  
    uint16_t ldr = analogRead(A0);
    uint16_t ntc = analogRead(A1);
    uint16_t mq2 = analogRead(A2);
    ...
    if (!mesh.write(&data, 'C', sizeof(data))) {
